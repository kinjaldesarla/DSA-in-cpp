#include<bits/stdc++.h>
using namespace std;

// 1. Single element in a Sorted Array   
//(even,odd) means element is on right side 
//(odd,even) means element is on left side
int single_element(int arr[],int n){
    if(n==1)return arr[0];
        if( arr[0]!=arr[1])return arr[0];
        if(arr[n-1]!=arr[n-2])return arr[n-1];
        int low=1,high=n-2;
        while(low<=high) {
            int mid=(low+high)/2;
            if(arr[mid]!=arr[mid-1] && arr[mid]!=arr[mid+1])return arr[mid];
            else if((mid%2==1 && arr[mid]==arr[mid-1])||(mid%2==0 && arr[mid]==arr[mid+1]))low=mid+1;
            else high=mid-1;
        }
     return -1;
}

// 2. Find peak element
int peak_element(int arr[],int n){
    if(n==1)return arr[0];
    if(arr[0]>arr[1])return arr[0];
    if(arr[n-1]>arr[n-2]) return arr[n-1];
    int low=1,high=n-2;
      while(low<=high) {
            int mid=(low+high)/2;
            if(arr[mid]>arr[mid-1] && arr[mid]>arr[mid+1])return arr[mid];
            else if(arr[mid]>arr[mid-1]) low=mid+1;
            else high=mid-1;
        }
        return -1;
}
int main(){
    int arr[]={1,1,2};
    cout<<single_element(arr,3);
    int peak[]={1,2,1,3,4,5,3,2};
    cout<<peak_element(peak,8);
    return 0;
}